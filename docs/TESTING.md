...
| `lib/redis-rest.ts` | distributed limiter and provider-budget tests | Unit | Complete Redis pair resolution, bounded responses, deployment-scoped HMAC counter keys, atomic integer counters, exact one-minute/24-hour TTL mapping, and fail-closed missing/malformed/unavailable behavior; geocoder result data is never a Redis value |
