...
| D10 | Production geocoder/provider selection (Panchangam #233) | Guest production search must not use public Nominatim. The older authenticated profile form still uses its disclosed public-Nominatim path and needs a no-regression migration. | Select and approve a managed provider, configure server-only `GEOCODER_BASE_URL` and `GEOCODER_USER_AGENT` for guest search, then migrate the authenticated profile path with journey tests. Guest production code rejects the public Nominatim host and cannot fall back to the legacy path. |
