...
export function CompatibilityClient({
  ...
  const selectedP1 = initialProfiles.find((p) => p.id === selection.p1);
  const selectedP2 = initialProfiles.find((p) => p.id === selection.p2);
    ...
    <div className="max-w-5xl mx-auto space-y-8 py-6 px-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {checks.map((c) => {
          ...
          let parsed: any = null;
          try { parsed = JSON.parse(c.result_json); } catch {}
          return (
            <Card
              key={c.id}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
          <div className="bg-zinc-950 border border-white/10 rounded-xl w-full max-w-2xl max-h-[90vh] flex flex-col shadow-2xl">
                  ...
                  {checks.length >= 6 && (
                    <div className="p-4 bg-amber-950/30 border border-amber-800/50 rounded-lg flex items-start gap-3">
                      ...
                      <div className="text-sm text-amber-200/90">
                        You have reached your limit of 6 compatibility checks. Please contact support or delete existing checks to run new ones.
                      </div>
                    </div>
                  )}
            ...
            <footer className="p-5 border-t border-white/10 text-center">
              {result ? (
                <Button onClick={() => { setResult(null); setSelection({}); setCalcError(null); }} variant="outline" className="w-full">
                  Run Another Check
                </Button>
              ) : (
